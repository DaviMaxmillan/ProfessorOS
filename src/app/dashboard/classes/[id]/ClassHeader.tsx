...
export default function ClassHeader({ classData }: { classData: any }) {
  ...
  const handleDelete = async () => {
    if (confirm('Tem certeza que deseja excluir esta turma e TODOS os dados (alunos, notas, frequências) vinculados a ela? Essa ação não pode ser desfeita.')) {
      setIsDeleting(true)
      ...
      <div className="page-header" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <form onSubmit={handleUpdate} style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <input 
            ...
            required 
            placeholder="Nome do Semestre (Ex: 1º Semestre/2026)"
            style={{ padding: '8px', borderRadius: '4px', border: '1px solid var(--surface-border)', background: 'rgba(0,0,0,0.3)', color: '#fff', width: '250px' }}
          />
  ...
  return (
    <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
      <div>
        ...
        <p className="page-description">{classData.semester.name} • {classData.professor}</p>
      </div>
